...
<div class="text-center mt-3">
  <a href="browse.php" class="btn btn-primary" id="browseBtn">Browse More</a>
</div>
...
function attachCategoryClick() {
  ...
  const workersContainer = document.getElementById("workers-containers");

  categoryButtons.forEach((btn) => {
    btn.addEventListener("click", () => {
      workersContainer.style.display = "block";

      // ✅ Smoothly scroll to workers section
      setTimeout(() => {
        const yOffset = -80; // adjust if your navbar covers content
        const y = workersContainer.getBoundingClientRect().top + window.pageYOffset + yOffset;
  ...
  fetch("fetch_workers.php", {
    ...
    .then((text) => {
      ...
      if (parsed && parsed.html !== undefined) {
        ...
        const workerCards = workersContainer.querySelectorAll(".worker-card"); // ensure your worker HTML uses .worker-card
  ...
  window.attachCategoryClick = function () {
    ...
    categoryButtons.forEach((btn) => {
      btn.addEventListener("click", function () {
        ...
        browseBtn.href = `browse.php?jobs[]=${job_id}`;
